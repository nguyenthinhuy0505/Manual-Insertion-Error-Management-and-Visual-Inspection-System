"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ImagesService = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
const bull_1 = require("@nestjs/bull");
const image_schema_1 = require("./schemas/image.schema");
const queue_constants_1 = require("../queue/queue.constants");
const events_gateway_1 = require("../gateway/events.gateway");
let ImagesService = class ImagesService {
    imageModel;
    inferenceQueue;
    eventsGateway;
    constructor(imageModel, inferenceQueue, eventsGateway) {
        this.imageModel = imageModel;
        this.inferenceQueue = inferenceQueue;
        this.eventsGateway = eventsGateway;
    }
    async uploadFromDevice(file, user, productId, location) {
        const image = await this.imageModel.create({
            filePath: file.path,
            source: 'aoi_machine',
            uploadedBy: user._id,
            status: 'pending',
            productId,
            location,
        });
        this.eventsGateway.emitImageProcessing(image);
        await this.inferenceQueue.add(queue_constants_1.JOB_INFERENCE, {
            imageId: image._id.toString(),
            filePath: file.path,
            productId,
            location,
        });
        return image;
    }
    async uploadFromInspector(file, user, productId, location) {
        const image = await this.imageModel.create({
            filePath: file.path,
            source: 'inspector',
            uploadedBy: user._id,
            status: 'pending',
            productId,
            location,
        });
        this.eventsGateway.emitImageProcessing(image);
        await this.inferenceQueue.add(queue_constants_1.JOB_INFERENCE, {
            imageId: image._id.toString(),
            filePath: file.path,
            productId,
            location,
        });
        return image;
    }
    async findAll(options) {
        const filter = {};
        if (options.status)
            filter.status = options.status;
        const skip = (options.page - 1) * options.limit;
        const [data, total] = await Promise.all([
            this.imageModel
                .find(filter)
                .populate('uploadedBy', 'username role')
                .populate('predictedLabel', 'code name severity')
                .populate('reviewedLabel', 'code name severity')
                .populate('reviewedBy', 'username')
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(options.limit)
                .exec(),
            this.imageModel.countDocuments(filter),
        ]);
        return { data, total, page: options.page, limit: options.limit };
    }
    async findById(id) {
        const image = await this.imageModel
            .findById(id)
            .populate('uploadedBy', 'username role')
            .populate('predictedLabel reviewedLabel reviewedBy');
        if (!image)
            throw new common_1.NotFoundException('Image not found');
        return image;
    }
    async updatePrediction(imageId, predictedLabel, confidence, isUnknown) {
        await this.imageModel.findByIdAndUpdate(imageId, {
            predictedLabel,
            confidence,
            isUnknown,
        });
    }
};
exports.ImagesService = ImagesService;
exports.ImagesService = ImagesService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_1.InjectModel)(image_schema_1.Image.name)),
    __param(1, (0, bull_1.InjectQueue)(queue_constants_1.QUEUE_INFERENCE)),
    __metadata("design:paramtypes", [mongoose_2.Model, Object, events_gateway_1.EventsGateway])
], ImagesService);
//# sourceMappingURL=images.service.js.map